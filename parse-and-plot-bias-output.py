...
parser.add_argument("--save", action="store_true", help="Include this if you want to save the plots to png file.")
...
if args.dataset == "leica_full":
    b_a_rmse_cut = rmse(gt.time[gt.time > 25], gt.bias_acc[gt.time > 25], data.time[data.time > 25], data.bias_acc[data.time > 25])
    b_g_rmse_cut = rmse(gt.time[gt.time > 25], gt.bias_gyr[gt.time > 25], data.time[data.time > 25], data.bias_gyr[data.time > 25])
...
if args.dataset.startswith("leica"):
    b_a_rmse_cut_str = '\t'.join(f"{d:.4f}" for d in b_a_rmse_cut)
    b_g_rmse_cut_str = '\t'.join(f"{d:.4f}" for d in b_g_rmse_cut)
    print(f" b_a_cut [m/s^2] = {b_a_rmse_cut_str}\n b_g_cut [rad/s] = {b_g_rmse_cut_str}")
...
plt.title("Position")
plt.plot(gt.time, gt.pos[:, 0], "--", color="gray", label="GT")
plt.plot(data.time, data.pos[:, 0], "b", label="Est")
ax.set_xticklabels([])
plt.ylabel("x [m]")
plt.legend(ncol=2)
ax = plt.subplot(3, 1, 2)
plt.plot(gt.time, gt.pos[:, 1], "--", color="gray")
plt.plot(data.time, data.pos[:, 1], "b")
ax.set_xticklabels([])
plt.ylabel("y [m]")
ax = plt.subplot(3, 1, 3)
plt.plot(gt.time, gt.pos[:, 2], "--", color="gray")
...
plt.plot(gt.time, gt.ypr_as_deg()[:, 2], "--", color="gray")
plt.plot(data.time, data.ypr_as_deg()[:, 2], "b")
ax.set_xticklabels([])
plt.ylabel("roll [deg]")
ax = plt.subplot(3, 1, 2)
plt.plot(gt.time, gt.ypr_as_deg()[:, 1], "--", color="gray")
plt.plot(data.time, data.ypr_as_deg()[:, 1], "b")
ax.set_xticklabels([])
plt.ylabel("pitch [deg]")
ax = plt.subplot(3, 1, 3)
plt.plot(gt.time, gt.ypr_as_deg()[:, 0], "--", color="gray")
...
if args.quat:
    ...
    plt.plot(gt.time, gt.quat[:, 0], "--", color="gray")
    plt.plot(data.time, data.quat[:, 0], "b")
    ax.set_xticklabels([])
    plt.ylabel("x")
    ax = plt.subplot(4, 1, 2)
    plt.plot(gt.time, gt.quat[:, 1], "--", color="gray")
    plt.plot(data.time, data.quat[:, 1], "b")
    ax.set_xticklabels([])
    plt.ylabel("y")
    ax = plt.subplot(4, 1, 3)
    plt.plot(gt.time, gt.quat[:, 2], "--", color="gray")
    plt.plot(data.time, data.quat[:, 2], "b")
    ax.set_xticklabels([])
    plt.ylabel("z")
    ax = plt.subplot(4, 1, 4)
    plt.plot(gt.time, gt.quat[:, 3], "--", color="gray")
...
plt.plot(gt.time, gt.bias_acc[:, 0], "--", color="gray")
plt.plot(data.time, data.bias_acc[:, 0], "b")
plt.plot(data.time, np.mean(data.bias_acc[:, 0]) * np.ones_like(data.time), "g--")
ax.set_xticklabels([])
plt.ylabel("x [m/s^2]")
ax = plt.subplot(3, 1, 2)
plt.plot(gt.time, gt.bias_acc[:, 1], "--", color="gray")
plt.plot(data.time, data.bias_acc[:, 1], "b")
plt.plot(data.time, np.mean(data.bias_acc[:, 1]) * np.ones_like(data.time), "g--")
ax.set_xticklabels([])
plt.ylabel("y [m/s^2]")
ax = plt.subplot(3, 1, 3)
plt.plot(gt.time, gt.bias_acc[:, 2], "--", color="gray")
plt.plot(data.time, data.bias_acc[:, 2], "b")
plt.plot(data.time, np.mean(data.bias_acc[:, 2]) * np.ones_like(data.time), "g--")
plt.ylabel("z [m/s^2]")
plt.xlabel("time [sec]")
plt.tight_layout()
if args.save:
...
plt.plot(gt.time, gt.bias_gyr[:, 0], "--", color="gray")
plt.plot(data.time, data.bias_gyr[:, 0], "b")
plt.plot(data.time, np.mean(data.bias_gyr[:, 0]) * np.ones_like(data.time), "g--")
ax.set_xticklabels([])
plt.ylabel("x [rad/s]")
ax = plt.subplot(3, 1, 2)
plt.plot(gt.time, gt.bias_gyr[:, 1], "--", color="gray")
plt.plot(data.time, data.bias_gyr[:, 1], "b")
plt.plot(data.time, np.mean(data.bias_gyr[:, 1]) * np.ones_like(data.time), "g--")
ax.set_xticklabels([])
plt.ylabel("y [rad/s]")
ax = plt.subplot(3, 1, 3)
plt.plot(gt.time, gt.bias_gyr[:, 2], "--", color="gray")
plt.plot(data.time, data.bias_gyr[:, 2], "b")
plt.plot(data.time, np.mean(data.bias_gyr[:, 2]) * np.ones_like(data.time), "g--")
plt.ylabel("z [rad/s]")
plt.xlabel("time [sec]")
plt.tight_layout()
if args.save:
...
if args.dataset.startswith("leica"):
    ...
    if args.dataset == "leica_skip40":
        ...
        plt.plot(data.time[data.time > 25], np.mean(data.bias_acc[data.time > 25, 0]) * np.ones_like(data.time[data.time > 25]), "g--")
    else:
        plt.plot(data.time[data.time > 25], np.mean(data.bias_acc[data.time > 25, 0]) * np.ones_like(data.time[data.time > 25]), "g--")
    ...
    if args.dataset == "leica_skip40":
        ...
        plt.plot(data.time[data.time > 25], np.mean(data.bias_acc[data.time > 25, 1]) * np.ones_like(data.time[data.time > 25]), "g--")
    else:
        plt.plot(data.time[data.time > 25], np.mean(data.bias_acc[data.time > 25, 1]) * np.ones_like(data.time[data.time > 25]), "g--")
    ...
    if args.dataset == "leica_skip40":
        ...
        plt.plot(data.time[data.time > 25], np.mean(data.bias_acc[data.time > 25, 2]) * np.ones_like(data.time[data.time > 25]), "g--")
    else:
        plt.plot(data.time[data.time > 25], np.mean(data.bias_acc[data.time > 25, 2]) * np.ones_like(data.time[data.time > 25]), "g--")
